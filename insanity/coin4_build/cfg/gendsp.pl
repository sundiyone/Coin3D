...
$build_dir = "/home/ed/Coin3D/coin4_build";
while ( $build_dir =~ m%[\\/][^\\/]*[^\\/.][\\/]\.\.% ) {
  $build_dir =~ s%[\\/][^\\/]*[^\\/\.][\\/]\.\.%%g;
}
$build_dir =~ s%[\\/]\.$%%g;
$src_dir = "/home/ed/Coin3D/coin4_build/../coin4";
while ( $src_dir =~ m%[\\/][^\\/]*[^\\/.][\\/]\.\.% ) {
  $src_dir =~ s%[\\/][^\\/]*[^\\/\.][\\/]\.\.%%g;
}
$src_dir =~ s%[\\/]\.$%%;
...
sub LoadVars {
  local $filename = "/home/ed/Coin3D/coin4_build/gendsp-vars.txt";
  open(IN, "${filename}");
  while (<IN>) {
    $line = $_;
    chop($line);
...
if ( $ARGV[0] eq "--register-public-header" ) {
  ...
  $sim_ac_relative_src_dir_q = "../coin4";
