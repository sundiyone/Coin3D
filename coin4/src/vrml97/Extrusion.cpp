...
static const bool ALWAYS_CREATE_TRIANGLES = false;
...
public:
  ...
  bool operator==(const SoVRMLExtrusionVertex & v) {
    return
      (this->coord == v.coord) &&
...
SoVRMLExtrusion::GLRender(SoGLRenderAction * action)
{
  ...
  bool vbo = SoVBO::shouldCreateVBO(state, contextid, PRIVATE(this)->coord.getLength());
  ...
  bool doTextures = SoMultiTextureEnabledElement::get(state);
...
calculate_y_axis(const SbVec3f * spine, const int i,
                 const int numspine, const bool closed)
{
  SbVec3f Y;
  if (closed) {
    if (i > 0) {
...
SoVRMLExtrusionP::generateCoords(void)
{
  ...
  int i, j, numcross;
  bool connected = false;   // is cross section closed
  bool closed = false;      // is spine closed
